const content = {
    heding: (
        <>
            Helping the people <br /> who need it most
        </>
    ),

    desc_one: (
        <>
            The unemployment rate rose by 12% during the pandemic in the urban area and remains uncertain for the rural
            population
        </>
    ),

    desc_two: (
        <>
            <span className="font-wt-700">Cosmanity</span> is a nonprofit platform helping those people by giving them a
            job, so they can support their families and themselves as they move forward
        </>
    ),
};

const videoCarousel = [
    {
        image: "https://i3.ytimg.com/vi/lsyYTdsBAmc/hqdefault.jpg",
        video_link: "https://youtu.be/lsyYTdsBAmc",
    },
    {
        image: "https://i3.ytimg.com/vi/X2mq-JvytRM/hqdefault.jpg",
        video_link: "https://youtu.be/X2mq-JvytRM",
    },
    {
        image: "http://i3.ytimg.com/vi/KgcxK4jnbVg/hqdefault.jpg",
        video_link: "https://youtu.be/KgcxK4jnbVg",
    },
    {
        image: "https://i3.ytimg.com/vi/cNnkiyXca3o/hqdefault.jpg",
        video_link: "https://youtu.be/cNnkiyXca3o",
    },
    {
        image: "https://i3.ytimg.com/vi/-EYcQgKU-tg/hqdefault.jpg",
        video_link: "https://youtu.be/-EYcQgKU-tg",
    },
    {
        image: "https://i3.ytimg.com/vi/o3tJ-8bc168/hqdefault.jpg",
        video_link: "https://youtu.be/o3tJ-8bc168",
    },
];

module.exports = { content, videoCarousel };
